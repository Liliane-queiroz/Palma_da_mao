        ...
        <form method="POST" action="${pageContext.request.contextPath}/cadastro-produto" enctype="multipart/form-data"> 
                ...
                <c:choose>
                    <c:when test="${eEdicao}">
                        <p class="aviso-foto">
                            ...
                            Foto não pode ser alterada nesta versão. Para trocar a foto, delete e recrie o anúncio.
                        </p>
                    </c:when>
                    <c:otherwise>
            ...
            <div class="form-group"> 
                ...
                <input type="text" id="nome" name="nome" required maxlength="150" 
                       placeholder="Ovos caipira" value="${eEdicao ? produtoParaEditar.nome : ''}"> 
            </div> 
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" id="preco" name="preco" step="0.01" required 
                           placeholder="0.00" value="${eEdicao ? produtoParaEditar.precoEstimado : ''}"> 
                </div> 
                
                <div class="form-group">
                    <label for="quantidade">Quantidade <span class="campo-obrigatorio">*</span></label> 
                    <input type="text" id="quantidade" name="quantidade" required 
                           placeholder="10" value="${eEdicao ? estoque.quantidade : ''}"> 
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select id="unidade" name="unidade" required> 
                        ...
                        <option value="UN" <c:if test="${eEdicao && estoque.unidade == 'UN'}">selected</c:if>>Unidade (UN)</option> 
                        <option value="KG" <c:if test="${eEdicao && estoque.unidade == 'KG'}">selected</c:if>>Quilograma (KG)</option> 
                        <option value="L" <c:if test="${eEdicao && estoque.unidade == 'L'}">selected</c:if>>Litro (L)</option> 
                        <option value="DZ" <c:if test="${eEdicao && estoque.unidade == 'DZ'}">selected</c:if>>Dúzia (DZ)</option> 
                        <option value="MAÇO" <c:if test="${eEdicao && estoque.unidade == 'MAÇO'}">selected</c:if>>Maço</option> 
                    </select> 
                </div>
                
                <div class="form-group">
                    <label for="data-entrega">Data prevista de entrega</label> 
